...
getSymbols("AMD", from= '2005-01-01', auto.assign = TRUE)

price_msft <- data.frame(AMD)

price_msft$Date <- rownames(price_msft) # replace rownames and create Date column
rownames(price_msft) <- NULL

price_msft$daily = 0
price_msft$daily[2:nrow(price_msft)] <-  log(price_msft$AMD.Adjusted[1:(nrow(price_msft)-1)]/price_msft$AMD.Adjusted[2:(nrow(price_msft))])
