...
get_daily_returns=function(symbol){
  print(as.character(symbol))
  price_daily <- data.frame(getSymbols(symbol, from= '2005-01-01', auto.assign = FALSE))
  price_daily$Date <- rownames(price_daily) # replace rownames and create Date column
  colnames(price_daily) <- c("Open", "High", "Low", "Close", "Volume", "Adjusted", "Date")
